...
def lambda_handler(event, context):
    ...
    This handler uses the single-user rotation scheme to rotate a Redshift user credential. This rotation
...
def set_secret(service_client, arn, token):
    ...
    if conn:
        conn.close()
        logger.info(
            "setSecret: AWSPENDING secret is already set as password in Redshift DB for secret arn %s."
            % arn
        )
        return
...
def test_secret(service_client, arn, token):
    ...
    if conn:
        ...
        logger.info(
            "testSecret: Successfully signed into Redshift DB with AWSPENDING secret in %s."
            % arn
        )
        return
    else:
...
def get_connection(secret_dict):
    ...
    Returns:
        Connection: The Redshift Connection object if successful. Raise error otherwise.
