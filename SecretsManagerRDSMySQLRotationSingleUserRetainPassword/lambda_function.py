...
def lambda_handler(event, context):
    ...
    This handler uses the master-user rotation scheme to rotate an RDS MySQL user credential. This rotation scheme
    logs into the database as the master user and rotates the user's password, retaining its previous password
    as a secondary password that is still valid for authenticating, thus aims high availability.
    ...
    The Secret SecretString is expected to be a JSON string with the following format:
    {
        ...
        'masterarn': <required: the arn of the master secret which will be used to change passwords>
    }
...
def set_secret(service_client, arn, token):
    ...
    tries to login with the master credentials from the masterarn in the current secret. If this succeeds, it sets the
    AWSPENDING password as the user password in the database. Else, it throws a ValueError.
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("SET PASSWORD FOR %s = " + password_option + " RETAIN CURRENT PASSWORD", (pending_dict['username'], pending_dict['password']))
            conn.commit()
